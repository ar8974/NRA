...
<div class="max-w-2xl mx-auto p-6 bg-white rounded shadow">
    ...
    <form action="{{ route('admin.schedules.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="Ep_ID" class="border p-2 w-full rounded" required>
                ...
                @foreach($episodes as $ep)
                    <option value="{{ $ep->Ep_ID }}" @selected(old('Ep_ID') == $ep->Ep_ID)>
        ...
        <div>
            ...
            <input type="datetime-local" name="Ep_Start" value="{{ old('Ep_Start') }}" class="border p-2 w-full rounded" required>
            @error('Ep_Start')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="datetime-local" name="Ep_End" value="{{ old('Ep_End') }}" class="border p-2 w-full rounded" required>
            @error('Ep_End')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" name="Viewers" value="{{ old('Viewers') }}" min="0" class="border p-2 w-full rounded" required>
            @error('Viewers')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex gap-2">
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Add Schedule</button>
