...
@section('content')
<h1 class="h4 mb-4">Edit Schedule</h1>
...
<form action="{{ route('admin.schedules.update', $schedule) }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <select name="Ep_ID" class="form-select" required>
            ...
            @foreach($episodes as $ep)
                <option value="{{ $ep->Ep_ID }}" @selected(old('Ep_ID', $schedule->Ep_ID) == $ep->Ep_ID)>{{ $ep->Title }} ({{ $ep->webseries->Name ?? 'N/A' }})</option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        <input type="datetime-local" name="Ep_Start" value="{{ old('Ep_Start', $schedule->Ep_Start) }}" class="form-control" required>
        @error('Ep_Start') <p class="text-danger small">{{ $message }}</p> @enderror
    </div>

    <div class="mb-3">
        <label class="form-label">End Date &amp; Time</label>
        <input type="datetime-local" name="Ep_End" value="{{ old('Ep_End', $schedule->Ep_End) }}" class="form-control" required>
    ...
    <div class="mb-3">
        ...
        <input type="number" name="Viewers" value="{{ old('Viewers', $schedule->Viewers) }}" min="0" class="form-control" required>
