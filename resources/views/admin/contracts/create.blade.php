...
@section('content')
<h1 class="h4 mb-4">Add New Contract</h1>
...
<form action="{{ route('admin.contracts.store') }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <input type="date" name="Contract_date" value="{{ old('Contract_date') }}" class="form-control" required>
        @error('Contract_date') <p class="text-danger small">{{ $message }}</p> @enderror
    </div>

    <div class="mb-3">
        <label class="form-label">End Date</label>
        <input type="date" name="Contract_end" value="{{ old('Contract_end') }}" class="form-control" required>
        @error('Contract_end') <p class="text-danger small">{{ $message }}</p> @enderror
    </div>

    <div class="mb-3">
        <label class="form-label">Charge per Episode</label>
        <input type="number" step="0.01" name="Charge_per_ep" value="{{ old('Charge_per_ep') }}" class="form-control" required>
